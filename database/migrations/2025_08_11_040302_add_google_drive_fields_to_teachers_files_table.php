...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('teachers_files', function (Blueprint $table) {
            if (!Schema::hasColumn('teachers_files', 'file_size')) {
                $table->unsignedBigInteger('file_size')->nullable()->after('file_type');
            }
            if (!Schema::hasColumn('teachers_files', 'google_drive_id')) {
                $table->string('google_drive_id')->nullable()->after('file_size');
            }
            if (!Schema::hasColumn('teachers_files', 'google_drive_link')) {
                $table->string('google_drive_link')->nullable()->after('google_drive_id');
            }
            if (!Schema::hasColumn('teachers_files', 'stored_in_drive')) {
    ...
    public function down(): void
    {
        Schema::table('teachers_files', function (Blueprint $table) {
            $drop = array_filter(
                ...
                fn($c) => Schema::hasColumn('teachers_files', $c)
            );
            if ($drop) {
