...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('type_of_art_files', function (Blueprint $table) {
            if (!Schema::hasColumn('type_of_art_files', 'file_size')) {
                $table->unsignedBigInteger('file_size')->nullable()->after('file_type');
            }
            if (!Schema::hasColumn('type_of_art_files', 'google_drive_id')) {
                $table->string('google_drive_id')->nullable()->after('file_size');
            }
            if (!Schema::hasColumn('type_of_art_files', 'google_drive_link')) {
                $table->string('google_drive_link')->nullable()->after('google_drive_id');
            }
            if (!Schema::hasColumn('type_of_art_files', 'stored_in_drive')) {
    ...
    public function down(): void
    {
        Schema::table('type_of_art_files', function (Blueprint $table) {
            $drop = array_filter(
                ...
                fn($c) => Schema::hasColumn('type_of_art_files', $c)
            );
            if ($drop) {
