...
window._incConfig = {
    ...
    entityStoreUrl:     "{{ route('management-incomes.entity.store') }}",
    entityRenameBase:   "{{ url('management-incomes/entity') }}",
...
<div class="py-12" x-data="incomeGrid()">
    <div class="max-w-full px-4 sm:px-6 md:px-8">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Ingresos por Gestión</h1>
                <p class="mt-1 text-sm text-gray-600">
                    Define las entidades, expande un mes y haz clic en una celda de entidad para registrar los ingresos diarios.
                </p>
            </div>
            <form method="GET" action="{{ route('management-incomes.index') }}" style="display:flex;align-items:center;gap:0.5rem;">
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2 p-3 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <template x-for="ent in entities" :key="ent.id">
                <div class="inline-flex items-center gap-1 rounded-full text-xs font-medium transition-colors"
                     :class="renamingEntityId === ent.id
                         ? 'px-1.5 py-0.5 bg-indigo-100 border border-indigo-400'
                         : 'px-2.5 py-1 bg-indigo-50 border border-indigo-200 text-indigo-800'">

                    <template x-if="renamingEntityId !== ent.id">
                        <div class="flex items-center gap-1.5">
                            <button @click="startRenameEntity(ent.id, ent.name)"
                                    class="hover:text-indigo-600 transition-colors cursor-text"
                                    title="Clic para renombrar"
                                    x-text="ent.name"></button>
                            <button @click="deleteEntity(ent.id)"
                                    class="text-indigo-300 hover:text-red-600 transition-colors leading-none"
                    ...
                    <template x-if="renamingEntityId === ent.id">
                        <div class="flex items-center gap-1">
                            <input :id="'rename-entity-' + ent.id"
                                   ...
                                   class="w-24 px-1.5 py-0.5 border border-indigo-400 rounded-full text-xs focus:outline-none focus:ring-1 focus:ring-indigo-400 bg-white">
                            <button @click="saveRenameEntity(ent.id)" :disabled="renamingEntity"
                                    class="text-indigo-600 hover:text-indigo-800 disabled:opacity-50" title="Guardar">
            ...
            <template x-if="!addingEntity">
                <button @click="addingEntity = true; $nextTick(() => $refs.newEntityInput.focus())"
                        class="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-600 text-white rounded-full text-xs font-medium hover:bg-indigo-700 transition-colors">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <template x-if="addingEntity">
                <div class="flex items-center gap-1.5">
                    <input x-ref="newEntityInput" x-model="newEntityName" type="text"
                           placeholder="Nombre (ej. Sede A)…"
                           @keydown.enter.prevent="addEntity()"
                           @keydown.escape.prevent="addingEntity = false; newEntityName = ''"
                           class="w-36 px-2 py-1 border border-indigo-400 rounded-full text-xs focus:outline-none focus:ring-1 focus:ring-indigo-400">
                    <button @click="addEntity()"
                            class="px-2.5 py-1 bg-indigo-600 text-white rounded-full text-xs font-medium hover:bg-indigo-700">
                        Añadir
                    </button>
            ...
            <template x-if="entities.length === 0">
                <span class="text-xs text-gray-400 italic">Añade entidades para registrar ingresos (ej. Sede A, Sede B).</span>
            </template>
                        ...
                        <tr class="bg-gray-800 text-white text-xs">
                            ...
                            <template x-for="col in visibleColumns()" :key="col.key">
                                <th :class="{
                                        ...
                                        'bg-indigo-600 !text-white ring-2 ring-inset ring-indigo-300': col.type === 'month' && col.mes === currentMonth && !col.expanded,
                                        'bg-indigo-600 !text-white ring-2 ring-inset ring-indigo-300': col.type === 'month' && col.mes === currentMonth && col.expanded,
                                        'cursor-pointer hover:bg-gray-700': col.type === 'month',
                                        'bg-green-700 !text-white min-w-[90px]': col.type === 'entity',
                                    }">
                                    ...
                                    <template x-if="col.type === 'month'">
                                        <button @click="toggleMonth(col.mes)"
                                            ...
                                            <span class="flex items-center gap-1">
                                                ...
                                                <svg x-show="col.expanded" class="w-3 h-3 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <template x-if="col.type === 'entity'">
                                        <div class="flex items-center justify-center gap-1">
                                            <span x-text="col.entityName" class="text-xs font-semibold text-green-100 uppercase tracking-wider"></span>
                                            <button @click="deleteEntity(col.entityId)"
                                                    class="text-green-300 hover:text-red-300 transition-colors flex-shrink-0"
                                                    title="Eliminar entidad">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/>
                                                </svg>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <template x-if="items.length === 0">
                            <tr><td :colspan="2 + visibleColumns().length + 2" class="px-6 py-14 text-center text-sm text-gray-400">
                                ...
                                <template x-if="entities.length === 0">
                                    <span> Añade entidades primero para poder registrar ingresos.</span>
                                </template>
                            </td></tr>
                        </template>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <template x-for="col in visibleColumns()" :key="col.key">
                                    <td :class="{
                                            ...
                                            'px-1 py-1 text-center bg-green-50/40': col.type === 'entity',
                                        }">

                                        {{-- Month cell: read-only total, click to expand --}}
                                        <template x-if="col.type === 'month'">
                                            <button @click="toggleMonth(col.mes)"
                                        ...
                                        <template x-if="col.type === 'entity'">
                                            <button @click="openEntityCalendar(col.entityId, item, col.mes)"
                                                    ...
                                                    :class="getEntityAmount(col.entityId, item, col.mes) > 0
                                                        ...
                                                        : 'text-gray-300 hover:bg-green-50 hover:text-gray-400'">
                        ...
                        <tr>
                            ...
                            <template x-for="col in visibleColumns()" :key="col.key">
                                <td :class="{
                                        ...
                                        'bg-indigo-50': col.type === 'month' && col.mes === currentMonth,
                                        'text-green-700 bg-green-50/60': col.type === 'entity',
                                    }">
                                    <template x-if="col.type === 'month'">
    ...
    <div x-show="calModal.open"
        ...
        <div @click.stop
            ...
            <div x-show="calModal.loading" style="display:flex;align-items:center;justify-content:center;padding:4rem 0;">
                <svg style="width:2rem;height:2rem;color:#4f46e5;animation:spin 1s linear infinite;" fill="none" viewBox="0 0 24 24">
            ...
            <div x-show="!calModal.loading" style="padding:1.25rem;">
                    ...
                    <template x-for="day in calModal.daysInMonth" :key="day">
                        <button @click="selectDay(day)"
                                ...
                                :style="calModal.selectedDay === day
                                    ? 'border-color:#4f46e5;background:#eef2ff;box-shadow:0 0 0 2px rgba(79,70,229,0.25);'
                                    : (getDayAmount(day) > 0
                            ...
                            <span style="font-size:0.75rem;font-weight:700;line-height:1;"
                                  :style="getDayAmount(day) > 0 ? 'color:#166534;' : 'color:#9ca3af;'"
                                  x-text="day"></span>
                            <span x-show="getDayAmount(day) > 0"
                                  style="font-size:0.65rem;font-weight:600;color:#166534;line-height:1;margin-top:auto;word-break:break-all;"
                ...
                <div x-show="calModal.selectedDay !== null"
                     ...
                     id="day-edit-panel"
                     style="margin-top:1rem;padding:1rem;background:#eef2ff;border:1.5px solid #a5b4fc;border-radius:10px;">
                    ...
                    <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
                        <div style="flex-shrink:0;">
                            ...
                            <input type="number" min="0" step="0.01"
                                   ...
                                   onfocus="this.style.borderColor='#4f46e5';this.style.boxShadow='0 0 0 2px rgba(79,70,229,0.15)'"
                    ...
                    <div style="display:flex;gap:0.5rem;margin-top:0.625rem;">
                        <button @click="saveDay(calModal.selectedDay)" :disabled="calModal.saving"
                                style="flex:1;padding:0.45rem 1rem;background:#4f46e5;color:#fff;border:none;border-radius:6px;font-size:0.8rem;font-weight:600;cursor:pointer;transition:background 0.1s;"
                                onmouseover="if(!this.disabled)this.style.background='#4338ca'" onmouseout="this.style.background='#4f46e5'">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-top:1rem;padding-top:0.75rem;border-top:1px solid #e5e7eb;">
                    ...
                    <span style="font-size:1.125rem;font-weight:700;color:#166534;">
        ...
        async saveDay(day) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Recalculate main grid cell (sum across all entities)
                    let crossTotal = 0;
                    for (const ent of this.entities) {
