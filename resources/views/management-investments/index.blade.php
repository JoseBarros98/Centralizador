...
window._invConfig = {
    ...
    detailUrl:   "{{ route('management-investments.monthDetail') }}",
    amountField: 'investment_amount',
};
</script>
<div class="py-12" x-data="investmentGrid()">
    <div class="max-w-full px-4 sm:px-6 md:px-8">

        {{-- Header --}}
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Inversiones en Otras Sedes por Gestión</h1>
                <p class="mt-1 text-sm text-gray-600">Haz clic en una celda para registrar la inversión diaria del mes.</p>
            </div>
            <form method="GET" action="{{ route('management-investments.index') }}" class="flex items-center gap-2">
                ...
                <table class="min-w-full text-sm border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <template x-for="item in filteredItems()" :key="item">
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-2 text-right font-bold text-indigo-700 border-l border-gray-100">
                    ...
                    <tfoot class="border-t-2 border-gray-300 bg-gray-50">
                        <tr>
                            ...
                            @endfor
                            <td class="px-4 py-3 text-right text-sm font-bold text-indigo-800 border-l border-gray-200">
        ...
        <div @click.stop
            ...
            <div x-show="!calModal.loading" style="padding:1.25rem;">
                    ...
                    <template x-for="day in calModal.daysInMonth" :key="day">
                        <button @click="selectDay(day)"
                                ...
                                :style="calModal.selectedDay === day
                                    ? 'border-color:#6366f1;background:#eef2ff;box-shadow:0 0 0 2px rgba(99,102,241,0.25);'
                                    : (getDayAmount(day) > 0
                            ...
                            <span style="font-size:0.75rem;font-weight:700;line-height:1;"
                                  :style="getDayAmount(day) > 0 ? 'color:#4338ca;' : 'color:#9ca3af;'"
                                  x-text="day"></span>

                            {{-- Amount (abbreviated) --}}
                            <span x-show="getDayAmount(day) > 0"
                                  style="font-size:0.65rem;font-weight:600;color:#4338ca;line-height:1;margin-top:auto;word-break:break-all;"
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;margin-top:1rem;padding-top:0.75rem;border-top:1px solid #e5e7eb;">
                    ...
                    <span style="font-size:1.125rem;font-weight:700;color:#4338ca;">
...
function investmentGrid() {
    ...
    return {
        ...
        selectDay(day) {
            ...
            this.$nextTick(() => {
                const amtEl = document.getElementById('day-edit-amount-inv');
                const obsEl = document.getElementById('day-edit-obs-inv');
                if (amtEl) { amtEl.value = existing ? existing.amount : ''; amtEl.focus(); amtEl.select(); }
                if (obsEl) obsEl.value = existing ? (existing.obs ?? '') : '';
                document.getElementById('day-edit-panel-inv')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            });
        },
